...
export function ProjectSettingsPage() {
  ...
  const createCard = useCreateCard(slug ?? "")
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-[22px] leading-tight font-extrabold tracking-tight">
            {projectDetail?.project.name} — Settings
          </h1>
          <p className="mt-1 text-[13px] text-muted-foreground">
            Create cards and manage board settings.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-2xl space-y-5">
        <section className="rounded-xl border border-border/60 bg-card p-5">
          <h2 className="text-[15px] font-semibold">Create a card</h2>
          <p className="mt-1 text-[12.5px] text-muted-foreground">
            Add a requirement card directly to the board.
          </p>
